...
### 5-3. 신발 종류 선택 (등록 화면)
- 신발 등록 `app/shoe/new.tsx`의 **종류(category)** 를 자유 입력에서 **칩 선택**으로 변경: **[러닝화 / 운동화 / 구두 / 기타]**.
- **기타** 선택 시에만 추가 텍스트 입력칸이 나타나 사용자가 직접 입력.
- 저장 값(`shoe.category`): 프리셋 선택 시 그 라벨, 기타 선택 시 입력한 텍스트. (데이터 모델 불변 — `category TEXT` 유지)
- 기타를 골랐는데 입력칸이 비어 있으면 저장 차단(또는 `category=null`로 저장) — 빈 "기타"는 의미 없으므로 입력 요구.
...
대상: `app/(tabs)/stats.tsx` 전면 개편 (현재는 "이번 달" 요약만 제공).

### 7-1. 기간 선택
- 상단 세그먼트: **[이번 달 / 올해 / 전체 / 사용자 지정]**
- **사용자 지정** 선택 시 **시작일·종료일**을 날짜 선택기(§6-1 재사용)로 지정 → 해당 구간으로 집계.
  - 제약: 시작 ≤ 종료, 미래 금지. 어기면 안내 메시지.
- 선택한 기간이 이후 모든 집계(요약·그래프)에 적용된다.

### 7-2. 구성 요소 (전부 포함)
1. **요약 카드** — 총 거리, 총 착용 횟수 (선택 기간 기준)
2. **🏆 가장 많이 신은 신발** — 착용 횟수 기준 강조 카드
3. **막대그래프 — 신발별 누적 거리** (내림차순)
4. **막대그래프 — 월별 거리 추이**

### 7-3. 차트 구현
- 막대그래프는 **별도 차트 라이브러리 없이 React Native `View`로 구현** (가로/세로 막대 = 너비/높이 비율, 테마 accent 색 적용). 새 의존성 없음 → SDK 54·Expo Go 안전.
- 집계 로직(기간 필터, 신발별 합계, 월별 버킷, 최다 착용)은 순수 함수 `src/domain/stats.ts`로 분리하여 **TDD**.

### 7-4. 데이터
- 기존 `getShoes('all')` + `getWearLogs()`로 충분 (추가 쿼리 불필요). 집계는 메모리에서 계산.
...
- **도메인(TDD)**: `src/domain/dates.ts` 신규(형식 검증·미래 금지·경과일), `src/domain/stats.ts` 신규(기간 필터·집계), 기존 `validation.ts`에 통합.

## 10. 테스트

- 도메인 날짜 로직은 **TDD** (Jest): 형식 유효/무효, 미래 금지 경계(오늘=통과, 내일=실패), 빈 값 처리, `daysSince` 경과일(같은 날=0, 과거=양수, 없음=null).
- 통계 집계(`src/domain/stats.ts`)도 **TDD**: 기간 필터(이번 달/올해/전체/사용자 지정 경계), 신발별 합계, 월별 버킷, 최다 착용, 빈 데이터.
...
- ✅ 설정 탭(4번째) + 8색 포인트 선택
- ✅ 내 신발 은퇴 필터 + 은퇴 해제 도달성 + "구매 후 N일" 표시
- ✅ 신발 종류 칩 선택(러닝화/운동화/구두/기타), 기타는 직접 입력
- ✅ 날짜 선택기 + 미래 금지 검증
- ✅ 통계 화면 상세화 (기간 선택[이번 달/올해/전체/사용자 지정] + 요약 + 최다 착용 + 막대그래프 2종)
- ✅ 전 화면 다크 테마 적용
...
- 🔜 선/도넛 등 고급 차트 (필요 시 react-native-svg 추가)
